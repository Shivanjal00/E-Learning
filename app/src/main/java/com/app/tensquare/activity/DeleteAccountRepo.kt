...
class DeleteAccountRepo  @Inject constructor(private val remoteDataSource: RemoteDataSource) :
    BaseApiResponse() {

    suspend fun deleteAccount(): Flow<NetworkResult<DeleteAccountResponse>> {
        return flow<NetworkResult<DeleteAccountResponse>> {
            emit(safeApiCall { remoteDataSource.deleteUserAccount() })
